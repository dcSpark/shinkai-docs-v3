---
title: 'OpenRouter'
description: 'Access 200+ AI models through a single API key'
icon: 'robot'
---

**OpenRouter AI models** offer a unified platform to access various advanced AI models from multiple providers. It simplifies integration, allowing developers to efficiently choose and deploy models for tasks like text generation and natural language processing, making it ideal for building versatile AI-powered applications.

<Tip>
For an overview of all cloud AI providers in Shinkai, see our [Cloud AI Models](/advanced/models/cloud-models) page.
</Tip>
  <Info> 
    You need to log in into [OpenRouter](https://openrouter.ai/settings/keys) to generate your API keys.
    Most models are paid, but you can find some free options. 
  </Info>
  <Steps>
    <Step title="Generate your API key">
      - Log in to OpenRouter > Hover your profile menu at the upper-right corner > Select `Keys`.
      - Click the `Create Key` button. 
      - Give your key a name (e.g., Shinkai) > Click `Create`.
      
      ![Generate key OpenRouter](/images/openrouter-key.jpg)

      <Icon icon="copy" iconType="duotone" /> Copy the key and go to your Shinkai app > Paste the key.
    </Step>
    <Step title="Select the model to install">
      OpenRouter offers over 200 AI models to choose from. We are going to choose the **free** model: `Meta: Llama 3.2 11B`.
      
      <Icon icon="copy" iconType="duotone" /> Copy the model id (see image)
      
      ![model id](/images/model-id.jpg)

    </Step>
    <Step title="Complete the manual installation in your Shinkai app">
      <Icon icon="paste" iconType="duotone" /> Paste the `model id` in the `Custom Model Type` field.
      
      In `AI Name` field you can customize how you want to see this AI displayed in your Chats. 
      <Note> This field only accepts alphanumeric characters and underscores. </Note>
      Click `Add AI`. You are all set! 

      ![Openrouter New AI](/images/openrouter-new-ai.jpg)

    </Step>
  </Steps>


