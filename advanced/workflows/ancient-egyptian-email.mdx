...
- Port: Your provider's IMAP port (typically uses SSL/TLS)
...
- Port: Your provider's SMTP port (typically uses SSL/TLS)
- Username: Your email address
- Password: Your email password or app-specific password

Note: Refer to your email provider's documentation for the specific server addresses and port numbers. Many providers also recommend using app-specific passwords for enhanced security.
...
const answeredEmailsQuery = await shinkaiSqliteQueryExecutor({
    query: 'SELECT * FROM answered_emails WHERE processed = ?',
    params: [false]
});
