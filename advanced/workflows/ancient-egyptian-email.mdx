...
The email processing follows a structured loop that handles each email individually, with built-in error handling and response tracking. Here's how the response generation works:

1. **Email Content Formatting**
   The tool structures each email's content in a clear XML format for the LLM:
   ```typescript
   <email>
     <email.subject>${email.subject}</email.subject>
     <email.sender>${email.sender}</email.sender>
...
2. **Personality Customization**
   The response_context field in the tool's configuration defines the assistant's personality. You can customize this in the Tool Config page:
   ```typescript
   ...
   Modify this context to create different personalities. The LLM will consistently style responses according to your configuration. For example:
   - Change the character's name and background
   - Adjust the tone (formal, casual, mysterious)
   - Add specific phrases or mannerisms
   - Include knowledge about specific topics
   
3. **Response Processing and Tracking**
   The tool ensures consistent and reliable response handling:

   - **Format Control**:
     ```typescript
     const response = await shinkaiLlmPromptProcessor({
         format: 'text',  // Ensures clean, plain text responses
   ...
   - **Response Tracking**:
     ```typescript
     // Record response in database with parameterized query
     await shinkaiSqliteQueryExecutor({
         query: 'INSERT INTO answered_emails (email_unique_id, subject, email, response) VALUES (?, ?, ?, ?)',
         params: [emailUniqueId, email.subject, email.sender, response.message]
     });
   ...
   - **Error Handling**:
     - Each email is processed in an isolated try-catch block
     - Failed responses are logged but don't block other emails
     - Duplicate responses are prevented via unique email IDs
     - The tool skips problematic emails and continues processing
     - No partial responses are ever sent to users
...
3. Set up scheduled execution:
   1. Navigate to Scheduled Tasks:
      - Click the clock icon with circling arrow in the Shinkai Desktop sidebar
      - You'll see a list of your existing scheduled tasks

   2. Create a new task:
      - Click the "Create New" button (highlighted in red)
      - In the Edit Scheduled Task form:
         * Task Name: Give it a descriptive name (e.g., "Khufur Mail")
         * Task Description: (Optional) Add details about the task
         * Task Prompt: Specify email date ranges (e.g., "Please answer emails from_date 23-Jan-2025")
         * Schedule: Choose frequency (e.g., "every 5 min" or custom cron expression)
         * AI/Agent: Select your preferred model (e.g., "gpt_4o_mini")
         * Tool: Select your Email Assistant from the dropdown

   3. Monitor task execution:
      - View task status in the main Scheduled Tasks list
      - Click "View Logs" to check execution history
      - Use the ellipsis menu (⋮) for additional options:
