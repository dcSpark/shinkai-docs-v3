...
<Steps titleSize="h3">
  ...
  <Step title="Paste the Keys in Your Shinkai Profile">
    ...
    - Node address: Enable the `Use Proxy`option and add `relayer_pub_01`. 

    ![Adding Public Keys to Shinkai Identity](/images/proxy-node.jpg)
  ...
  <Step title="Copy the name of your registered Shinkai Identity">
    ...
    - Click on the Shinkai Identity input box, input your identity name, and click save. Make sure to include the `@@` prefix so your identity looks like `@@myidentity.sep-shinkai`.
...
Once you have connected your identity, you can verify the connection by clicking  the `Verify Shinkai Identity`button below your Shikai Identity (`Settings` > `General`).

You will be redirected to your profile page, and if everything is OK, you will see a green success message:

<Icon icon="check" iconType="duotone" />`Your encryption and signature public keys are correctly in sync.`
