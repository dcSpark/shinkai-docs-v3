...
You can see the capabilities of this Cardano AI Agent in this video (link).

This tutorial is a detailed overview of the AI Agent design and implementation process, but not a step by step guide on how to code all of its tools.
...
This tool retrieves and presents Cardano NFT market data. It retrieves overall market statistics (volume, buyers, sellers, and percentage changes), it identifies today top NFT collections based on customizable ranking criteria (and fetches additional collection details like social links), it calculates total trading volume over specified timeframes, and presents lists of top collections by volume. It delivers personalized and user-friendly summaries of the Cardano NFT market landscape.
...
Once you have defined your blcokchain AI Agent's tools, you can build them carefully following the guidance below to make them optimally useful.
...
But a tool with too many timeframe inputs might confuse the LLM of the AI Agent, as well as timeframe names too similar.
...
You should also remove from the output some data coming from the data sources but that you’re not interested in. This will improve speed and lower cost of the LLM inference.
...
* pick a performant LLM able to interpret complex outputs (e.g. gpt4o). You can also choose of a LLM with vision task capabilities, for example if you want your blockchain AI Agent to analyse trading charts.
...
These prompts are from the preview screenshots of our Taptools Cardano tools available in the Shinkai AI Store. You can see most of our Cardano AI Agent answers on these screenshots on their respective pages. Some prompts and answers can also be viewed in the demo video (link).
...
And you can find 2 examples of Telegram bot deployment in these videos ([NASA Agent demo](https://youtu.be/Yct9NNzXlXM?feature=shared&t=709), link).
